...
describe("Emails", () => {
  ...
  test("Only pro users can access emails", async () => {
    await t.mutation(internal.users.upsert, {
      clerk_id: "userid1",
      workspace: "workspace1",
      roles: ["org:admin"],
    ...
    try {
      await tAuth1.query(api.emails.search, {
        paginationOpts: {
          numItems: 10,
          cursor: null,
        },
    ...
    try {
      await tAuth2.mutation(api.emails.create, {
        title: "Template 1",
      });
      expect(true).toBe(false);
    } catch (error) {
    ...
    try {
      await tAuth2.query(api.emails.search, {
        paginationOpts: {
          numItems: 10,
          cursor: null,
        },
